...
=head1 SYNOPSYS
 ...
 Usage: qc <command> [<working directory>]
...
(http://daringfireball.net/projects/markdown/). It considers
...
You may just put HTML files as source. Quietly Confident
will remove anything not needed as: anything between C<<head>..</head>>,
C<<html>> and C<<body>> tags. It will also remove any C<<font>> or C<<span>> tags
...
be installed using CPAN:
	...
	$ cpan
	cpan[1]> install Git::Repository
	cpan[1]> install Text::Markdown
	cpan[1]> install Moo
	cpan[1]> install Template
	cpan[1]> install HTTP::Server::Brick
	cpan[1]> install Config::General
	cpan[1]> install IO::All
...
=head1 Details on source tree organisation
...
=head1 Using a git repository as the source
...
=head1 What kind of weird name is this for a static site generator?
